/**
 * @module components/sections/AnimationGallery
...
/**
 * @component
 * @description Renders a comprehensive gallery of Framer Motion animation patterns including
 * entrance choreography, hover effects, staggered lists, animated disclosures, presence toggles,
 * and draggable prototypes. Used on the homepage to showcase animation capabilities.
 *
 * @returns {JSX.Element} Section element containing multiple animation demonstration subsections
 *
 * @example
 * <AnimationGallery />
 */
...
import type { MotionFadeInDirection } from '@components/motion/MotionFadeIn';
import { MotionHoverCard } from '@components/motion/MotionHoverCard';
import { MotionStaggerList } from '@components/motion/MotionStaggerList';
import { MotionTogglePanel } from '@components/motion/MotionTogglePanel';
import { MotionDraggableCard } from '@components/motion/MotionDraggableCard';
  ...
  const sections: AnimationGallerySection[] = [
          ...
          <AnimatePresence>
            {showToast && (
              <motion.div
                ...
                className="bg-success-600 inline-flex items-center gap-3 rounded-full px-4 py-2 text-sm font-medium text-white shadow-lg"
              >
    ...
    },
    {
      ...
      content: (
        <div
          ref={dragBoundsRef}
          className="border-secondary-400 bg-secondary-50 dark:border-secondary-700 dark:bg-secondary-900/40 flex min-h-[220px] w-full items-center justify-center rounded-2xl border border-dashed p-6"
        >
          <MotionDraggableCard
